...
- [x] 7.4 Human visual pass on a real document: confirm A4 density reads well and no line/image/table row is split at a page boundary (GUI-only; the geometry is verified but glyph-level fidelity needs eyes). Verified with `/Users/user/Documents/运营与供应链管理/复习重点/重点-03-啤酒游戏与牛鞭效应/啤酒游戏-案例解答_Codex.md` exported to `tmp/pdfs/save-as-pdf-regression/beer-final-after-fix.pdf`; Poppler-rendered boundary and page checks showed the original page 1/2 text split fixed and long table rows splitting only between rows.
