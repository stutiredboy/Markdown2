...
Detect inline (`$...$`, `\(...\)`) and block (`$$...$$`, `\[...\]`, and bare `\begin{<env>}...\end{<env>}`) TeX math in Markdown source, emit it safely into the preview HTML, and typeset it offline with KaTeX so technical and academic documents render correctly in the Read-mode preview without any network access.
