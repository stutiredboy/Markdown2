...
- **图片附件** — 粘贴截图会将其保存到可配置的、相对于文档的文件夹（默认 `assets/`）；拖入或粘贴图片文件则直接链接到该文件的现有位置（不复制）。可用 Pandoc 风格的 `{width=480}` / `{width=320 height=180}` 属性控制尺寸；本地图片缺失时预览中会显示明确的损坏占位提示。
- **导出与打印** — 将渲染后的文档导出为分页 PDF（含可导航的标题大纲/书签树），或直接打印。可配置的**导出预设**控制页面大小（A4 / Letter / Legal）、方向（纵向 / 横向）、页边距（正常 / 窄 / 宽 / 无，或自定义），以及可选的页码与页眉/页脚文本（支持 `{title}`、`{date}`、`{page}`、`{pageCount}` 占位符）——默认仍为 A4 纵向窄边距，保持原有输出不变。导出与打印结果中的图表与预览一致。
- **导出为 HTML** — 导出为单个自包含 `.html` 文件，样式、数学/图表引擎与图片全部内联，可离线打开且无需附带文件。DOCX/EPUB 转换后端已为后续版本保留，但当前不会在菜单中暴露这些命令。
- **快捷模式切换** — 编辑模式按 `Esc` 切换到预览，预览模式 `Cmd+双击` 切回编辑，或从工具栏选择「双栏」同时查看编辑和预览。
- **双向滚动同步** — 在双栏模式下，滚动任一窗格时另一窗格自动对齐到相同内容。
- **查找与替换** — `Cmd+F` 在编辑器或预览中打开查找栏；支持上一个/下一个导航、替换单个或全部匹配项。
- **行号** — 可选择在编辑模式、预览模式或两者中显示源码行号。编辑模式在正文左侧的行号栏中标注每一行（软换行的行只标注一次）；预览模式在每个渲染块左侧显示其起始源码行号，因此阅读时看到的行号就是在编辑器中要打开的行。两种范围都可在工具栏或设置中开关；行号仅用于显示，不会出现在导出的 PDF、打印或 HTML 中。
- **应用设置** 支持语言、默认打开模式（编辑 / 双栏 / 预览）、默认大纲可见性、行号（编辑与预览），以及导出预设（页面大小、方向、页边距、页码、页眉/页脚）。
